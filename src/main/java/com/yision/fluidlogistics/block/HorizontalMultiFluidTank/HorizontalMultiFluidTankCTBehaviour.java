...
public class HorizontalMultiFluidTankCTBehaviour extends ConnectedTextureBehaviour.Base {

    protected CTSpriteShiftEntry topShift;
    protected CTSpriteShiftEntry layerShift;
    private CTSpriteShiftEntry innerShift;

    public HorizontalMultiFluidTankCTBehaviour(CTSpriteShiftEntry layerShift, CTSpriteShiftEntry topShift, CTSpriteShiftEntry innerShift) {
    ...
    public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
        if (sprite != null && direction.getAxis() == state.getValue(HorizontalMultiFluidTankBlock.AXIS) && innerShift.getOriginal() == sprite)
            return innerShift;
        return direction.getAxis() == state.getValue(HorizontalMultiFluidTankBlock.AXIS) ? topShift : layerShift;
    }

    @Override
    protected Direction getUpDirection(BlockAndTintGetter reader, BlockPos pos, BlockState state, Direction face) {
        Direction.Axis vesselAxis = state.getValue(HorizontalMultiFluidTankBlock.AXIS);
