...
public final class FluidGaugeHelper {
    ...
    public static int clampRestockThreshold(int threshold) {
        return Mth.clamp(threshold, DEFAULT_RESTOCK_THRESHOLD, MAX_FLUID_AMOUNT);
    }

    public static int clampPromiseLimit(int limit) {
        return Mth.clamp(limit, DEFAULT_PROMISE_LIMIT, MAX_FLUID_AMOUNT);
    }

    public static int clampAdditionalStock(int amount) {
        return Mth.clamp(amount, DEFAULT_ADDITIONAL_STOCK, MAX_FLUID_AMOUNT);
    }
