...
public class MultiFluidTankScenes {
	...
	public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 4; i++) {
			scene.world().modifyBlockEntity(tankPos, MultiFluidTankBlockEntity.class, be -> {
				be.getTankInventory().drain(water.copyWithAmount(1000), FluidAction.EXECUTE);
			});
			scene.idle(5);
		}
		...
		for (int i = 0; i < 4; i++) {
			scene.world().modifyBlockEntity(tankPos, MultiFluidTankBlockEntity.class, be -> {
				be.getTankInventory().fill(water.copyWithAmount(1000), FluidAction.EXECUTE);
			});
			scene.idle(5);
		}
		scene.idle(40);
		...
		scene.world().modifyBlockEntity(util.grid().at(4, 3, 0), SpoutBlockEntity.class, be -> {
			...
			if (handler != null)
				handler.fill(chocolate.copyWithAmount(1000), FluidAction.EXECUTE);
		});
		...
		ItemStack chocolateBucket = AllFluids.CHOCOLATE.get()
			.getFluidType()
			.getBucket(chocolate.copyWithAmount(1000));
