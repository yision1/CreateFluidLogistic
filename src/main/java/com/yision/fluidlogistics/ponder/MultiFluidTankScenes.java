...
public class MultiFluidTankScenes {

	public static final String STORAGE = "multi_fluid_tank/storage";
	public static final String SIZES = "multi_fluid_tank/sizes";

	public static void storage(SceneBuilder builder, SceneBuildingUtil util) {
		CreateSceneBuilder scene = new CreateSceneBuilder(builder);
		RandomSource random = RandomSource.create();

		scene.title("multi_fluid_tank_storage", "Storing Multiple Fluids in Multi-Fluid Tanks");
		scene.configureBasePlate(0, 0, 5);
		scene.showBasePlate();
		scene.idle(5);

		BlockPos tankPos = util.grid().at(3, 1, 2);
		Selection fluids = util.select().fromTo(1, 5, 0, 0, 8, 1);
		...
		scene.idle(5);
		ElementLink<WorldSectionElement> fluidsLink =
			scene.world().showIndependentSection(fluids, Direction.NORTH);
		scene.world().moveSection(fluidsLink, util.vector().of(2, -4, 3), 0);
		scene.idle(10);

		scene.overlay().showOutline(PonderPalette.GREEN, fluidsLink,
			util.select().fromTo(3, 1, 3, 2, 4, 4), 40);
		scene.idle(10);
		...
		scene.overlay().showText(40)
			.text("Multi-Fluid Tanks can store up to eight fluid types in one structure")
			.attachKeyFrame()
			.placeNearTarget()
			.pointAt(util.vector().blockSurface(util.grid().at(3, 2, 1), Direction.WEST));
		scene.idle(50);

		scene.world().hideIndependentSection(fluidsLink, Direction.DOWN);
		scene.idle(5);
		FluidStack water = new FluidStack(Fluids.WATER, 6000);
		FluidStack lava = new FluidStack(Fluids.LAVA, 6000);
		FluidStack chocolate = new FluidStack(AllFluids.CHOCOLATE.get().getSource(), 6000);
		FluidStack honey = new FluidStack(AllFluids.HONEY.get().getSource(), 6000);
		scene.world().modifyBlockEntity(tankPos, MultiFluidTankBlockEntity.class, be -> {
			be.getTankInventory().fill(water, FluidAction.EXECUTE);
			be.getTankInventory().fill(lava, FluidAction.EXECUTE);
			be.getTankInventory().fill(chocolate, FluidAction.EXECUTE);
			be.getTankInventory().fill(honey, FluidAction.EXECUTE);
		});
		scene.idle(25);
		...
		scene.world().modifyBlock(pumpPos, state -> state.setValue(PumpBlock.FACING, Direction.SOUTH), false);
		scene.idle(10);
		scene.world().showSection(largeCog1, Direction.UP);
		scene.world().showSection(kinetics1, Direction.WEST);
		scene.idle(10);
		scene.world().setBlock(util.grid().at(1, -1, 5), AllBlocks.MULTI_FLUID_TANK.getDefaultState(), false);
		scene.world().setKineticSpeed(pump, -32);

		scene.idle(5);
		scene.world().propagatePipeChange(pumpPos);
		scene.world().modifyBlockEntity(util.grid().at(2, 0, 5), MultiFluidTankBlockEntity.class, be -> {
			be.getTankInventory().fill(water, FluidAction.EXECUTE);
			be.getTankInventory().fill(lava, FluidAction.EXECUTE);
			be.getTankInventory().fill(chocolate, FluidAction.EXECUTE);
			be.getTankInventory().fill(honey, FluidAction.EXECUTE);
		...
		for (int i = 0; i < 4; i++) {
			scene.world().modifyBlockEntity(tankPos, MultiFluidTankBlockEntity.class, be -> {
				be.getTankInventory().drain(FluidHelper.copyStackWithAmount(water, 1000), FluidAction.EXECUTE);
			});
			scene.idle(5);
		}
		...
		scene.world().modifyBlock(pumpPos, state -> state.setValue(PumpBlock.FACING, Direction.NORTH), true);
		scene.world().propagatePipeChange(pumpPos);
		for (int i = 0; i < 4; i++) {
			scene.world().modifyBlockEntity(tankPos, MultiFluidTankBlockEntity.class, be -> {
				be.getTankInventory().fill(FluidHelper.copyStackWithAmount(water, 1000), FluidAction.EXECUTE);
			});
			scene.idle(5);
		}
		scene.idle(40);
		...
		scene.overlay().showText(50)
			.text("A Comparator measures the combined fill level")
			.attachKeyFrame()
			.placeNearTarget()
		...
		scene.idle(80);
		scene.world().modifyBlockEntity(util.grid().at(4, 3, 0), SpoutBlockEntity.class, be -> {
			IFluidHandler handler = be.getCapability(ForgeCapabilities.FLUID_HANDLER).orElse(null);
			if (handler != null)
				handler.fill(FluidHelper.copyStackWithAmount(chocolate, 1000), FluidAction.EXECUTE);
		});
		...
		scene.overlay().showText(80)
			.text("Basins, Item Drains and Spouts can automate fluid container handling")
			.attachKeyFrame()
		...
		ItemStack chocolateBucket = AllFluids.CHOCOLATE.get()
			.getFluidType()
			.getBucket(FluidHelper.copyStackWithAmount(chocolate, 1000));
		scene.world().createItemOnBeltLike(util.grid().at(3, 1, 0), Direction.WEST, chocolateBucket);
		scene.idle(40);
		...
		scene.world().createItemOnBeltLike(util.grid().at(4, 1, 0), Direction.UP, chocolateBucket);
		for (int i = 0; i < 10; i++) {
			scene.effects().emitParticles(util.vector().topOf(3, 1, 1)
					.add(0, 1 / 16f, 0),
				scene.effects().simpleParticleEmitter(FluidFX.getFluidParticle(chocolate),
	...
	public static void sizes(SceneBuilder builder, SceneBuildingUtil util) {
		CreateSceneBuilder scene = new CreateSceneBuilder(builder);
		scene.title("multi_fluid_tank_sizes", "Dimensions of a Multi-Fluid Tank");
		...
		scene.overlay().showText(60)
			.text("Multi-Fluid Tanks can be combined to increase total capacity")
			.attachKeyFrame()
			.placeNearTarget()
		...
		scene.overlay().showText(60)
			.text("Their base can be up to 3 blocks wide...")
			.attachKeyFrame()
			.placeNearTarget()
			.pointAt(util.vector().topOf(2, 1, 2));
		scene.idle(40);
		...
		scene.overlay().showText(60)
			.text("...and grow up to 30 blocks tall")
			.attachKeyFrame()
			.placeNearTarget()
		...
		scene.idle(7);
		scene.world().modifyBlocks(full2,
			state -> state.setValue(MultiFluidTankBlock.SHAPE, MultiFluidTankBlock.Shape.PLAIN), false);
		scene.idle(30);

		scene.overlay().showText(60)
			.text("Using a Wrench, the tank windows can be toggled")
			.attachKeyFrame()
			.placeNearTarget()
			.pointAt(blockSurface);
		scene.idle(50);
