...
public class MechanicalFluidGunItemSelectionHandler {
	...
	public static void rightClickingBlocksSelectsTarget(PlayerInteractEvent.RightClickBlock event) {
		...
		while (iterator.hasNext()) {
			...
			if (selectedTarget.pos().equals(pos)) {
				...
				PacketDistributor.sendToServer(
					new MechanicalFluidGunPackets.ItemTargetSelectionPacket(pos.immutable(), null, true));
		...
		PacketDistributor.sendToServer(
			new MechanicalFluidGunPackets.ItemTargetSelectionPacket(pos.immutable(), face, false));
	...
	public static void leftClickingBlocksClearsTarget(PlayerInteractEvent.LeftClickBlock event) {
		...
		while (iterator.hasNext()) {
			...
			if (target.pos().equals(event.getPos())) {
				iterator.remove();
				PacketDistributor.sendToServer(
					new MechanicalFluidGunPackets.ItemTargetSelectionPacket(event.getPos().immutable(), null, true));
				event.setCanceled(true);
				return;
			}
		}
	...
	public static void tick(ClientTickEvent.Post event) {
		...
		if (!isHoldingGun(player)) {
			clearCurrentItemSelection(player);
			currentItem = null;
			currentSlot = NO_SELECTED_SLOT;
			selectedTargets.clear();
			return;
		...
		if (held != currentItem) {
			...
			rebuildTargetsFromItemNBT(held);
			return;
		}
	...
	private static void rebuildTargetsFromItemNBT(ItemStack stack) {
		selectedTargets.clear();
		Level level = Minecraft.getInstance().level;
		if (level == null) return;
		if (!MechanicalFluidGunItem.hasSelectedTargets(stack)) return;

		List<MechanicalFluidGunItem.SelectedTarget> nbtTargets = MechanicalFluidGunItem.getSelectedTargets(stack, level);
		for (MechanicalFluidGunItem.SelectedTarget target : nbtTargets) {
			selectedTargets.add(new TargetSelection(target.pos(), target.face()));
		}
	}
	...
	private static void clearCurrentItemSelection(Player player) {
		if (currentItem == null || selectedTargets.isEmpty() || currentSlot == NO_SELECTED_SLOT) {
			return;
		}

		PacketDistributor.sendToServer(
			MechanicalFluidGunPackets.ItemTargetSelectionPacket.clearSelectedTargets(currentSlot));
	}
