...
public interface PackageResourceDisplay {
    ...
    record FactoryPanelRestockPolicy(
        ...
        public int clampThreshold(int amount) {
            return configurableThreshold ? Math.max(0, Math.min(maxSettingAmount, amount)) : 0;
        }

        public int clampAdditionalStock(int amount) {
            return configurableAdditionalStock ? Math.max(0, Math.min(maxSettingAmount, amount)) : 0;
        }

        public int clampPromiseLimit(int amount) {
            return configurablePromiseLimit ? Math.max(-1, Math.min(maxSettingAmount, amount)) : -1;
        }
    ...
    default ItemStack icon(ItemStack normalizedKey) {
        ...
        ItemStack icon = normalizedKey.copy();
        icon.setCount(1);
        return icon;
    }
