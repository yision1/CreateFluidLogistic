...
class FaucetDrips {
    ...
    void update(IFluidHandler sourceHandler) {
        List<FluidStack> dripPreview = FluidSourceScans.previewDripFluids(
            sourceHandler, be::testFluidFilter, DRIP_AMOUNT, true);
        cache(dripPreview);
        apply(dripPreview);
    }
    ...
    private boolean applyPreview(List<FluidStack> dripPreview) {
        ...
        if (!contains(dripPreview, dripFluid)) {
            ...
            FluidStack preview = dripPreview.get(index);
            FluidStack nextDripFluid = FluidHelper.copyStackWithAmount(preview,
                Math.min(preview.getAmount(), DRIP_AMOUNT));
            boolean fluidChanged = !sameFluid(dripFluid, nextDripFluid);
    ...
    private void advance() {
        ...
        dripFluid = FluidHelper.copyStackWithAmount(nextFluid, Math.min(nextFluid.getAmount(), DRIP_AMOUNT));
    }
    ...
    private void cache(List<FluidStack> dripPreview) {
        ...
        for (FluidStack preview : dripPreview) {
            cachedDripFluids.add(FluidHelper.copyStackWithAmount(preview,
                Math.min(preview.getAmount(), DRIP_AMOUNT)));
        }
    }
    ...
    private void spawnParticle() {
        ...
        FluidLogisticsPackets.sendToNear(serverLevel, be.getBlockPos(), 32,
    ...
    void write(CompoundTag tag) {
        FaucetFilling.writeFluid(tag, TAG_DRIP_FLUID, dripFluid);
    ...
    void read(CompoundTag tag) {
        dripFluid = FaucetFilling.readFluid(tag, TAG_DRIP_FLUID);
