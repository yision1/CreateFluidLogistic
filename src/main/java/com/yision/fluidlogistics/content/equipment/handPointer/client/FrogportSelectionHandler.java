...
public class FrogportSelectionHandler {
    ...
    public static void drawChainContour(PoseStack ms, MultiBufferSource buffer, Vec3 camera) {
        ...
        while (!toVisit.isEmpty()) {
            ...
            if (mc.level.getBlockEntity(current) instanceof ChainConveyorBlockEntity be) {
                for (BlockPos connection : be.connections) {
                    BlockPos connectedPos = current.offset(connection);
                    if (!visited.contains(connectedPos) && AllBlocks.CHAIN_CONVEYOR.has(mc.level.getBlockState(connectedPos))) {
    ...
    public static boolean tryConnectCurrentTarget(Level level) {
        ...
        FluidLogisticsPackets.getChannel()
            .sendToServer(new HandPointerFrogportConnectionPacket(selectedFrogports,
