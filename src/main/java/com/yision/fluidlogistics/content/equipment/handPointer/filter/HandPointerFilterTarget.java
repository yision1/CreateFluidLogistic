...
public class HandPointerFilterTarget implements MenuProvider, IInteractionChecker {
    ...
    private final boolean requiresHandPointer;

    public HandPointerFilterTarget(BlockPos pos, Direction side, Vec3 hitLocation, ItemStack iconStack) {
        this(pos, side, hitLocation, iconStack, true);
    }

    public HandPointerFilterTarget(BlockPos pos, Direction side, Vec3 hitLocation, ItemStack iconStack,
        boolean requiresHandPointer) {
        this.pos = pos;
        this.side = side;
        this.hitLocation = hitLocation;
        this.iconStack = iconStack;
        this.requiresHandPointer = requiresHandPointer;
    }
    ...
    public boolean canPlayerUse(Player player) {
        ...
        if (requiresHandPointer && !(player.getMainHandItem().getItem() instanceof HandPointerItem)) {
            return false;
        }
    ...
    public void encode(RegistryFriendlyByteBuf buf) {
        ...
        buf.writeBoolean(requiresHandPointer);
    }

    public static HandPointerFilterTarget decodeClient(RegistryFriendlyByteBuf buf) {
        ...
        ItemStack iconStack = ItemStack.OPTIONAL_STREAM_CODEC.decode(buf);
        boolean requiresHandPointer = buf.readBoolean();
        return new HandPointerFilterTarget(pos, side, hitLocation, iconStack, requiresHandPointer);
    }
}
