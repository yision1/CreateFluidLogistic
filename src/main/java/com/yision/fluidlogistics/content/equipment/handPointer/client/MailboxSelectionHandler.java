...
public class MailboxSelectionHandler {
    ...
    public static BlockPos getSelectedMailboxPos() {
        return hasSelection() ? selectedMailboxes.get(0) : null;
    }
    ...
    public static boolean addMailbox(BlockPos pos, Player player, Level level) {
        if (selectedMailboxes.size() >= Config.getHandPointerMaxMailboxes()) {
            return false;
        }
        ...
        CreateLang.builder()
            .translate("fluidlogistics.hand_pointer.mailbox.added", selectedMailboxes.size(), Config.getHandPointerMaxMailboxes())
            .color(0xDDC166)
            .sendStatus(player);
        return true;
    }
    ...
    public static void renderSelection(Minecraft mc) {
        ...
        for (BlockPos mailboxPos : selectedMailboxes) {
            BlockState mailboxState = mc.level.getBlockState(mailboxPos);
            VoxelShape mailboxShape = mailboxState.getShape(mc.level, mailboxPos);
        ...
        boolean alreadyConnected = isCurrentTargetAlreadyConnected(mc.level);

        BlockState stationState = mc.level.getBlockState(selectedStationPos);
        VoxelShape stationShape = stationState.getShape(mc.level, selectedStationPos);
    ...
    public static boolean tryConnectCurrentTarget(Level level) {
        ...
        FluidLogisticsPackets.getChannel()
            .sendToServer(new HandPointerMailboxStationConnectionPacket(selectedMailboxes, selectedStationPos));
        return true;
    }
    ...
    public static boolean isCurrentTargetOutOfRange() {
        return isCurrentTargetOutOfRange(Minecraft.getInstance().level);
    }

    public static boolean isCurrentTargetOutOfRange(Level level) {
        if (!hasSelection() || selectedStationPos == null || level == null) {
            return false;
        }
